...
void handler(int s)
{
  ...
  if(s==SIGUSR2) {
    ...
    // s-commentate l'istruzione kill e osservate cosa succede
     kill(getpid(),SIGUSR1);	//OSS: succede che se manda da solo un segnale SIGUSR1, troveremo alla fine che ha avuto 1 segnale in più di quelli che gli abbiamo mandato noi
    continua = false;
  } 
}
...
int main(int argc, char *argv[])
{
  ...
  while(continua) { // loop apparentemente senza uscita
    ;               // questo e' un busy waiting: da evitare assolutamente.  Usare pause() per attendere il segnale 
    pause();
  }
